...
- [修复] Custom Webhook delivery now treats HTTP 201/202 responses as successful accepted sends and logs accepted message ids plus Resend `last_event` delivery status when available.
...
- [修复] 英文报告的通知标题、成交量、财务金额、每股分红与机构持股单位不再混用中文，统一按英文标题及 `K/M/B`、ISO 币种格式输出。
